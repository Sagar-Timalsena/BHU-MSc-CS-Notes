...
# start = time.time()
# bucket_sort(arr)
# avg_times.append((time.time() - start) * 1000)arr = generate_worst_case(size)
# start = time.time()
# bucket_sort(arr)
# worst_times.append((time.time() - start) * 1000)
# print(f"Size: {size:5d}, Best: {best_times[-1]:6.2f} ms, Avg: {avg_times[-1]:6.2f} ms, Worst:
# {worst_times[-1]:6.2f} ms")
# # Normalization
